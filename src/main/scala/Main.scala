...
object Main extends App {
  ...
  val flow = Flow[Goods].map(fractionize).via(CsvFormatting.format())

  val sink: Sink[Goods, Future[IOResult]] = Flow[Goods].map(fractionize).via(CsvFormatting.format()).toMat(FileIO.toPath(csvOutputPath))(Keep.right)

  val future = for {
    ioResult <- source.runWith(sink)
...
object Goods extends SQLSyntaxSupport[Goods] {
  ...
  def streamAll: Publisher[Goods] = DB readOnlyStream {
    sql"""select ${g.result.*} from ${Goods as g}"""
